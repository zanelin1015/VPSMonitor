...
func (s *SQLiteStore) updateAgentConfig(agentID string, cfg model.ManagedAgentConfig) (model.AgentRecord, error) {
	...
	_, err = s.db.Exec(`
		UPDATE agents
		SET agent_name = ?, agent_tags_json = ?, updated_at = ?, xui_config_json = ?, nezha_config_json = ?, renewal_config_json = ?, entry_config_json = ?
		WHERE agent_id = ?
	`,
		record.AgentName,
		mustJSON(record.Config.Tags),
...
func (s *SQLiteStore) ListAgents() ([]model.AgentRecord, error) {
	rows, err := s.db.Query(`
		...
		ORDER BY COALESCE(ls.reported_at, '') DESC, a.created_at DESC, a.agent_id ASC
	`)
	if err != nil {
	...
	for rows.Next() {
		...
		record.Config, err = s.parseManagedConfig(record.AgentID, record.AgentName, tagsJSON, xuiJSON, nezhaJSON, renewalJSON, entryJSON)
		if err != nil {
			return nil, err
		}
		record.Tags = cloneStrings(record.Config.Tags)
...
func (s *SQLiteStore) loadAgentQuery(db queryer, agentID string) (model.AgentRecord, bool, error) {
	...
	err := db.QueryRow(`
		SELECT agent_id, agent_name, agent_tags_json, agent_token, hostname, public_ipv4, public_ipv6,
		       created_at, updated_at, last_seen_at, xui_config_json, nezha_config_json, renewal_config_json, entry_config_json
		FROM agents
		WHERE agent_id = ?
	`, agentID).Scan(
